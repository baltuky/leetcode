...
public class Solution {
    public List<String> removeSubfolders(String[] paths) {
        final Set<String> haystack = new HashSet<>(Arrays.asList(paths));
        final List<String> result = new LinkedList<>();

        for (String path : paths) {
            if (!containsSubPath(haystack, path)) {
                result.add(path);
            }
        }
    ...
    private boolean containsSubPath(Set<String> paths, String path) {
        final StringBuilder prefix = new StringBuilder();
        final StringTokenizer tokenizer = new StringTokenizer(path, "/");

        while (tokenizer.hasMoreTokens()) {
            prefix.append("/").append(tokenizer.nextToken());
            if (paths.contains(prefix.toString()) && tokenizer.hasMoreTokens()) {
                return true;
            }
        }
        return false;
